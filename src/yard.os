...
//   
Процедура РаспаковатьАрхив(Знач ПутьКАрхиву, Знач КаталогРаспаковки)
...
Процедура УстановитьНовыйРелиз(Знач ПапкаСУстановочнымФайлом, Знач КаталогДляУстановки)

	ПапкаДляБитФинанс = "";

	КомандаУстановкиРелиза = ПапкаСУстановочнымФайлом + "\setup.exe /S " + КаталогДляУстановки;
	Сообщить("Команда для установки - " + КомандаУстановкиРелиза);
	ЗапуститьПриложение(КомандаУстановкиРелиза,,Истина);
	
	СнятьАтрибутФайлаТолькоДляЧтения(ПапкаСУстановочнымФайлом + "\" + ПапкаДляБитФинанс);

КонецПроцедуры // УстановитьНовыйРелиз()

Процедура СнятьАтрибутФайлаТолькоДляЧтения(Знач Папка)
	Файлы = НайтиФайлы(Папка,"*",Ложь);
	Для каждого Файл Из Файлы Цикл
		Ф = Новый Файл(Файл.ПолноеИмя);
...
ФильтрКонфигураций = "Библиотека стандартных";
ФильтрВерсий = "3\.1\.2";
...
Для Каждого ТекКонфигурация Из СписокКонфигураций Цикл
	...
	Для Каждого ТекВерсия Из СписокВерсий Цикл
		...
		Для Каждого ТекСсылка Из СписокСсылок Цикл
			...
			РаспаковатьАрхив(ИмяФайла, КаталогДляРаспаковки);
