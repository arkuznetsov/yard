...
Функция ДатаИзСтроки(Знач ДатаСтрокой) Экспорт
	...
	Попытка
		ДлинаГода = 4;
		ДлинаМесяцаДня = 2;
		Если СтрДлина(ВремЧастиДаты[2]) = ДлинаГода Тогда
			Возврат Дата(СтрШаблон("%1%2%3%4", ВремЧастиДаты[2], ВремЧастиДаты[1], ВремЧастиДаты[0], "000000"));
		ИначеЕсли СтрДлина(ВремЧастиДаты[2]) = ДлинаМесяцаДня Тогда
...
// Функция - Выполняет сравнение версий по соглашению SEMVER.
// Пример
//  (-1) - СтрокаВерсии1 меньше (<) СтрокаВерсии2
//  (1) - СтрокаВерсии1 больше (>) СтрокаВерсии2
//  (0) - СтрокаВерсии1 равна (=) СтрокаВерсии2
//
// Параметры:
//    СтрокаВерсии1   - Строка   - Строковое представление версии типа "1.0.0.0"
//    СтрокаВерсии2   - Строка   - Строковое представление версии типа "1.0.0.0"
//
// Возвращаемое значение:
//    Число   - результат сравнения в числе (0 = Равны, -1 = Меньше, 1 = Больше),
//              относительно первой версии (СтрокаВерсия1)
//
//
...
//
// Параметры:
//	МассивВерсий     - Массив из Строка    - массив номеров версий
...
Функция ФорматДвузначноеЧисло(Знач ЗначениеЧисло)
	...
	Если СтрДлина(ЧислоСтрокой) < ДлинаРезультата Тогда
...
Функция ДатаPOSIX(Знач Дата) Экспорт
	...
	Возврат СтрШаблон("%1-%2-%3 %4:%5:%6",
	                  Год(Дата),
	                  ФорматДвузначноеЧисло(Месяц(Дата)),
	                  ФорматДвузначноеЧисло(День(Дата)),
	                  ФорматДвузначноеЧисло(Час(Дата)),
	                  ФорматДвузначноеЧисло(Минута(Дата)),
	                  ФорматДвузначноеЧисло(Секунда(Дата)));
	
КонецФункции // ДатаPOSIX()

// Процедура - удаляет из текста HTML-страницы символы переноса строк,
// возврата каретки, пробелы и табуляции между тегами
//
// Параметры:
...
Процедура ОчиститьТекстСтраницыHTML(Текст) Экспорт
	...
	Совпадения = НайтиСовпаденияВТексте(Текст, Шаблон);
...
// Функция - декодирует разделители параметров в адресе, полученном из HTML
//
// Параметры:
//  Адрес    - Строка    - адрес из HTML-атрибута
//
// Возвращаемое значение:
//  Строка    - адрес с декодированными разделителями параметров
//
Функция ДекодироватьРазделителиПараметровHTML(Знач Адрес) Экспорт

	Возврат СтрЗаменить(Адрес, "&amp;", "&");

КонецФункции // ДекодироватьРазделителиПараметровHTML()

// Функция - добавляет параметр к адресу, сохраняя уже имеющуюся строку запроса
//
// Параметры:
//  Адрес   - Строка - адрес ресурса
//  Параметр - Строка - параметр в формате "имя=значение"
//
// Возвращаемое значение:
//  Строка - адрес с добавленным параметром
//
Функция ДобавитьПараметрЗапроса(Знач Адрес, Знач Параметр) Экспорт

	Разделитель = ?(СтрНайти(Адрес, "?") > 0, "&", "?");
	Возврат СтрШаблон("%1%2%3", Адрес, Разделитель, Параметр);

КонецФункции // ДобавитьПараметрЗапроса()
...
//
Функция НайтиСовпаденияВТексте(Текст, Шаблон) Экспорт
...
Процедура УбратьКавычки(Значение) Экспорт
	...
	Если Лев(Значение, 1) = """"  И Прав(Значение, 1) = """" Тогда
		Значение = Сред(Значение, Смещение, СтрДлина(Значение) - Смещение);
	КонецЕсли;
...
Процедура УдалитьПустыеКаталогиРекурсивно(Знач ПутьККаталогу, Знач ДоКаталога = "") Экспорт
	...
	ФайлыВКаталоге = НайтиФайлы(ПутьККаталогу, ПолучитьМаскуВсеФайлы());
...
//
// Параметры:
//  МассивФайлов     - Массив из Файл   - список файлов
//
// Возвращаемое значение:
...
Функция ОбщийПутьФайлов(МассивФайлов) Экспорт
	...
	Если НЕ (Прав(ОбщийПуть, 1) = "\" ИЛИ Прав(ОбщийПуть, 1) = "/") Тогда
		Поз = Макс(СтрНайти(ОбщийПуть, "\", НаправлениеПоиска.СКонца), СтрНайти(ОбщийПуть, "/", НаправлениеПоиска.СКонца));
		ОбщийПуть = Сред(ОбщийПуть, Поз);
	КонецЕсли;

	Если СтрДлина(ОбщийПуть) = 1 Тогда
		ОбщийПуть = "";
	КонецЕсли;
...
Функция ОписаниеРелиза(Знач Путь) Экспорт
	...
	УстановитьДатуИзСтрокиИСО(ОписаниеРелиза, "Дата");
...
// Процедура - конвертирует значение поля из строки формата ISO в дату (после неудачной конвертации из JSON)
//
// Параметры:
//  Приемник	- Структура	- Структура в которой надо заменить значение
//  ИмяСвойства	-	Строка	- Имя свойства с датой ISO
//
// <Описание процедуры>
//
// Параметры:
//   Приемник - <Тип.Вид> - <описание параметра>
//   ИмяСвойства - <Тип.Вид> - <описание параметра>
//
Процедура УстановитьДатуИзСтрокиИСО(Приемник, ИмяСвойства) Экспорт
	Если Приемник.Свойство(ИмяСвойства)
		И ТипЗнч(Приемник[ИмяСвойства]) = Тип("Строка")
		И ЗначениеЗаполнено(Приемник[ИмяСвойства]) Тогда
		Приемник[ИмяСвойства] = ПрочитатьДатуJSON(Приемник[ИмяСвойства], ФорматДатыJSON.ISO);
	КонецЕсли;
КонецПроцедуры
