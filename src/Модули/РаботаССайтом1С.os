//
// Реализация взаимодействия с сайтом 1С на основе обработки: https://infostart.ru/public/255881/
//
...
//
Функция ПолучитьСписокПриложений(ИдСеанса = "", Знач Фильтр = Неопределено, Знач Имя = "", Знач Пароль = "") Экспорт
	
	Если НЕ ЗначениеЗаполнено(ИдСеанса) Тогда
		ИдСеанса = Авторизация(СервисРелизов, Имя, Пароль, СтраницаСпискаРелизов);
	КонецЕсли;

	СтраницаКонфигураций = ПолучитьСтраницуСайта(СервисРелизов, СтраницаСпискаРелизов, ИдСеанса);
...
//
Функция ПолучитьВерсииПриложения(Знач АдресРесурса, Знач ИдСеанса, Знач Фильтр = Неопределено) Экспорт
	
	СтраницаВерсий = ПолучитьСтраницуСайта(СервисРелизов, АдресРесурса, ИдСеанса);
...
//
Функция ПолучитьСсылкиДляЗагрузки(Знач АдресРесурса = "", Знач ИдСеанса = "", Знач Фильтр = Неопределено) Экспорт
	
	СтраницаВерсии = ПолучитьСтраницуСайта(СервисРелизов, АдресРесурса, ИдСеанса);
	...
	Если Совпадения.Количество() > 0 Тогда
		...
		Для Каждого ТекСовпадение Из Совпадения Цикл
			...
			СтраницаЗагрузки = ПолучитьСтраницуСайта(СервисРелизов, ТекСсылка, ИдСеанса);
...
//
Процедура ЗагрузитьФайл(АдресИсточника, Знач Имя, Знач Пароль, Знач ПутьКФайлуДляСохранения) Экспорт
...
Процедура Инициализация()
	...
	ШаблонПоискаКонфигураций = "<td class=""nameColumn"">\s*<a href=""(.*)"">(.*)<\/a>";
