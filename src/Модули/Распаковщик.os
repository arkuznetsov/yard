...
Процедура ЗаписатьВыбранныеФайлы(ПутьКФайлуDeflate,
	...
	Если РаспаковкаКаталогов = Перечисления.ВариантыРаспаковкиКаталогов.БезОбщихКаталогов Тогда
		ОбщийПуть = ОбщийПутьФайлов(ФайлыВАрхиве);
	КонецЕсли;
...
//
Функция ОбщийПутьФайлов(ФайлыВАрхиве)

	ОбщийПуть = Лев(ФайлыВАрхиве[0].ПолноеИмя, СтрДлина(ФайлыВАрхиве[0].ПолноеИмя) - СтрДлина(ФайлыВАрхиве[0].Имя));
	Для й = 1 По ФайлыВАрхиве.ВГраница() Цикл
		Если ФайлыВАрхиве[й].Имя = ФайлыВАрхиве[й].ПолноеИмя Тогда
			Прервать;
		КонецЕсли;
		Для к = 1 По СтрДлина(ОбщийПуть) Цикл
			Если НЕ Сред(ОбщийПуть, к, 1) = Сред(ФайлыВАрхиве[й].ПолноеИмя, к, 1) Тогда
