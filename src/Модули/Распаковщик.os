...
// Ищет путь к установленной 7-zip
//
// Возвращаемое значение:
...
Функция Найти7ZIP()
	...
	ЭтоЛинукс = СтрНайти(НРег(Информация.ВерсияОС), "unix") > 0;
	ПутьДоАрхиватора = "";
	...
	Выхлоп = Команда.ПолучитьВывод();
	Если КодВозврата = 0 Тогда
		Если ЭтоЛинукс Тогда
			ПутьДоАрхиватора = ПолучитьЧистыйПутьК7ZIP(Выхлоп);
			Если ПустаяСтрока(ПутьДоАрхиватора) Тогда
				ПутьДоАрхиватора = "";
			КонецЕсли;
		Иначе
			ПутьДоАрхиватора = Выхлоп;
		КонецЕсли;
	КонецЕсли;

	Возврат ПутьДоАрхиватора;

КонецФункции // Найти7ZIP()

Функция ПолучитьЧистыйПутьК7ZIP(Знач ВходящееЗначение)
	ВходящееЗначение = СтрЗаменить(ВходящееЗначение, "7z:", ""); 
	ЧастиСтроки = СтрРазделить(СокрЛП(ВходящееЗначение), " ");
