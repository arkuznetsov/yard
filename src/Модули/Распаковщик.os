...
Процедура РаспаковатьШаблонКонфигурации1С(Знач ПутьКАрхивуEFD,
                                          ...
                                          Знач ВыбранныеФайлы = "") Экспорт

	ВремФайлАрхива = Новый Файл(ПутьКАрхивуEFD);

	ПутьКФайлуDeflate = ИзвлечьАрхивDeflate(ПутьКАрхивуEFD);
	
	ЗаписатьВыбранныеФайлы(ПутьКФайлуDeflate, КаталогРаспаковки, ВыбранныеФайлы);
...
Функция ИзвлечьАрхивDeflate(Знач ПутьКАрхивуEFD, Знач КаталогРаспаковки = "")
	...
	РаспаковатьАрхив(ПутьКФайлуZIP, КаталогРаспаковки);
...
//  ВыбранныеФайлы      - Строка         - имя файла или список файлов для распаковки
//                        Массив(Строка)   если не указан, то распаковываются все файлы
//  СохранятьИерархию   - Булево         - Истина - создавать каталоги при извлечении файлов,
//                                         как они указаны в архиве;
//                                         Ложь - файлы будут извлечены в каталог распаковки
//
Процедура ЗаписатьВыбранныеФайлы(ПутьКФайлуDeflate,
                                 Знач КаталогРаспаковки,
                                 Знач ВыбранныеФайлы = "",
                                 Знач СохранятьИерархию = Ложь)

	ФайлыВАрхиве = ПолучитьСписокФайловВАрхиве(ПутьКФайлуDeflate);

	МассивФайлов = Новый Массив();

	Если ТипЗнч(ВыбранныеФайлы) = Тип("Строка") И ЗначениеЗаполнено(ВыбранныеФайлы) Тогда
	...
	Для Каждого ТекСтрока Из ФайлыВАрхиве Цикл
		...
		ЗаписатьФайл(Поток, ТекСтрока, КаталогРаспаковки, СохранятьИерархию);
...
//                                       Ложь - файлы будут извлечены в каталог распаковки
//
Процедура ЗаписатьФайл(Поток, Знач ОписаниеФайла, Знач КаталогРаспаковки, Знач СохранятьИерархию = Ложь)
	...
	Если СохранятьИерархию Тогда
		ПутьКФайлу = ОбъединитьПути(КаталогРаспаковки, ОписаниеФайла.ПолноеИмя);
	Иначе
