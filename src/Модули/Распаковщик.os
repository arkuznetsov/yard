//
// Реализация распаковки EFD-файла на основе обработки: https://infostart.ru/public/618906/
//

#Область ПрограммныйИнтерфейс

Функция РаспаковатьШаблон(Знач АрхивШаблона, Знач ВыбранныеФайлы, Знач КаталогРаспаковки = "") Экспорт

	ВремФайлАрхива = Новый Файл(АрхивШаблона);

	ИмяСжатогоФайла	= СтрШаблон("%1_%2.ifl", ВремФайлАрхива.ИмяБезРасширения, Новый УникальныйИдентификатор()); // ВремФайл.Имя; 

	ПутьКФайлуZIP = DeflateToZIP(АрхивШаблона, ИмяСжатогоФайла);

	ВремФайлZIP = Новый Файл(ПутьКФайлуZIP);

	РаспаковатьАрхив(ПутьКФайлуZIP, ВремФайлZIP.Путь);

	УдалитьФайлы(ПутьКФайлуZIP);
			
	СтруктураАрхива = ПрочитатьСтруктуруАрхива(ОбъединитьПути(ВремФайлZIP.Путь, ИмяСжатогоФайла));

	Если НЕ ЗначениеЗаполнено(КаталогРаспаковки) Тогда
		КаталогРаспаковки = ВремФайлZIP.Путь;
	КонецЕсли;
	...
	ЗаписатьВыбранныеФайлы(ОбъединитьПути(ВремФайлZIP.Путь, ИмяСжатогоФайла), СтруктураАрхива, МассивФайлов, КаталогРаспаковки);

	Возврат СтруктураАрхива;

КонецФункции // РаспаковатьШаблон()

#КонецОбласти // ПрограммныйИнтерфейс

#Область СлужебныеПроцедуры

Функция DeflateToZIP(Знач ПутьКАрхивуDeflate, Знач ИмяСжатогоФайла, Знач ПутьКАрхивуZIP = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ПутьКАрхивуZIP) Тогда
		ВремФайлАрхива = Новый Файл(ПутьКАрхивуDeflate);
		ИмяФайлаZIP = СтрШаблон("%1_%2.zip", ВремФайлАрхива.ИмяБезРасширения, Новый УникальныйИдентификатор());
		ПутьКАрхивуZIP = ОбъединитьПути(ВремФайлАрхива.Путь, ИмяФайлаZIP);
	КонецЕсли;

	Бинарник = Новый ДвоичныеДанные(ПутьКАрхивуDeflate);
	...
	// Название файла
	Для й = 0 По ДлинаИмениСжатогоФайла - 1 Цикл
		БинарныйБуфер.Установить(ТекущееСмещение + ДлинаФиксированнойЧастиCDFH + й,
		                         КодСимвола(Сред(ИмяСжатогоФайла, й + 1, 1)));
	КонецЦикла;
...
#КонецОбласти // Deflate

Функция ПрочитатьСтруктуруАрхива(РаспакованныйФайл)

	Дерево = Новый ДеревоЗначений();
	Дерево.Колонки.Добавить("Имя"       , Новый ОписаниеТипов("Строка"));
	Дерево.Колонки.Добавить("Размер"    , Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 0)));
	Дерево.Колонки.Добавить("Смещение"  , Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 0)));
	Дерево.Колонки.Добавить("ЭтоКаталог", Новый ОписаниеТипов("Булево"));
	
	Бинарник = Новый ДвоичныеДанные(РаспакованныйФайл);
	Поток = Бинарник.ОткрытьПотокДляЧтения();
	Смещение = 0;
	...
	// ЧтоТоТам = ПрочитатьРазмер(Поток, Смещение);
	Смещение = Смещение + См4;
	
	КолВоЯзыковыхБлоков = ПрочитатьРазмер(Поток, Смещение);
	Смещение = Смещение + См4;
	
	Для й = 1 По КолВоЯзыковыхБлоков Цикл
		ДлинаЯзыковогоБлока = ПрочитатьРазмер(Поток, Смещение);
		Смещение = Смещение + ДлинаЯзыковогоБлока;
	КонецЦикла;
	
	КолВоФайлов = ПрочитатьРазмер(Поток, Смещение);
	Смещение = Смещение + См4;
	
	Для й = 1 По КолВоФайлов Цикл
		ДлинаБлокаИмениФайла	= ПрочитатьРазмер(Поток, Смещение);
		ДлинаИмениФайла			= ПрочитатьРазмер(Поток, Смещение + См4);
		ИмяФайла				= ПрочитатьИмяФайла(Поток, Смещение + См8, ДлинаБлокаИмениФайла - См8 - См16);
		
		Если ДлинаИмениФайла <> СтрДлина(ИмяФайла) Тогда
			Сообщить("Ошибка чтения имени файла №" + й);
			Продолжить;
		КонецЕсли;
		
		СмещениеФайла	= ПрочитатьРазмер(Поток, Смещение + ДлинаБлокаИмениФайла - См8);
		РазмерФайла		= ПрочитатьРазмер(Поток, Смещение + ДлинаБлокаИмениФайла - См4);
		Смещение = Смещение + ДлинаБлокаИмениФайла;
		
		ДобавитьВДерево(Дерево, ИмяФайла, РазмерФайла, СмещениеФайла);
	КонецЦикла;
...
КонецФункции // ПрочитатьСтруктуруАрхива()

Процедура ДобавитьВДерево(Дерево, Имя, Размер, Сдвиг)

	МнИмя = Имя;
	МнИмя = СтрЗаменить(МнИмя, "\", Символы.ПС);
	
	ВетвиНаУровне = Дерево.Строки;
	МаксУровень = СтрЧислоСтрок(МнИмя);
	
	Для Уровень = 1 По МаксУровень Цикл
		ТекФайл = СтрПолучитьСтроку(МнИмя, Уровень);
		ЭтоКаталог = Уровень <> МаксУровень;
		
		ПараметрыОтбора = Новый Структура("Имя", ТекФайл);
		НайденныеСтроки = ВетвиНаУровне.НайтиСтроки(ПараметрыОтбора);
		
		Если НайденныеСтроки.Количество() = 0 Тогда
			НовСтр = ВетвиНаУровне.Добавить();
			НовСтр.Имя        = ТекФайл;
			НовСтр.Размер     = Размер;
			НовСтр.Смещение   = ?(ЭтоКаталог, 0, Сдвиг);
			НовСтр.ЭтоКаталог = ЭтоКаталог;
			ВетвиНаУровне = НовСтр.Строки;
		Иначе
			НайденныеСтроки[0].Размер = НайденныеСтроки[0].Размер + Размер;
			ВетвиНаУровне = НайденныеСтроки[0].Строки;
		КонецЕсли;
	КонецЦикла;
...
Процедура НайтиСтрокиПоИмени(Дерево, Имя, ВыбранныеСтроки)

	Для Каждого ТекСтрока Из Дерево.Строки Цикл
		НайтиСтрокиПоИмени(ТекСтрока, Имя, ВыбранныеСтроки);
		Если ТекСтрока.Имя = Имя Тогда
			ВыбранныеСтроки.Добавить(ТекСтрока);
		КонецЕсли;
	КонецЦикла;

КонецПроцедуры // НайтиСтрокиПоИмени()

Процедура ЗаписатьВыбранныеФайлы(ФайлАрхива, ДеревоАрхива, ВыбранныеФайлы, Знач КаталогРаспаковки, СохранятьИерархию = Ложь)

	Бинарник = Новый ДвоичныеДанные(ФайлАрхива);
	Поток = Бинарник.ОткрытьПотокДляЧтения();
	
	НачальноеСмещение = 4;
	КолВоЯзыковыхБлоков = ПрочитатьРазмер(Поток, НачальноеСмещение);
	НачальноеСмещение = НачальноеСмещение + 4;
	...
	Для й = 1 По КолВоФайлов Цикл
		ДлинаБлокаИмениФайла	= ПрочитатьРазмер(Поток, НачальноеСмещение);
		НачальноеСмещение = НачальноеСмещение + ДлинаБлокаИмениФайла;
	КонецЦикла;

	Для Каждого ТекФайл Из ВыбранныеФайлы Цикл

		ВыбранныеСтроки = Новый Массив();
		НайтиСтрокиПоИмени(ДеревоАрхива, ТекФайл, ВыбранныеСтроки);
		...
		Для Каждого ТекСтрока Из ВыбранныеСтроки Цикл
			Смещение = НачальноеСмещение;
			Родитель = ТекСтрока.Родитель;
			МассивКаталогов = Новый Массив();
			Пока ЗначениеЗаполнено(Родитель) Цикл
				Смещение = Смещение + Родитель.Смещение;
				МассивКаталогов.Вставить(0, Родитель.Имя);
				Родитель = Родитель.Родитель;
			КонецЦикла;
			Если СохранятьИерархию Тогда
				Для Каждого ТекКаталог Из МассивКаталогов Цикл
					КаталогРаспаковки = ОбъединитьПути(КаталогРаспаковки, ТекКаталог);
				КонецЦикла;
			КонецЕсли;
			Если ТекСтрока.ЭтоКаталог Тогда
				ЗаписатьФайлыКаталога(Поток, Смещение, ТекСтрока, КаталогРаспаковки);
			Иначе 
				ЗаписатьФайл(Поток, Смещение, ТекСтрока, КаталогРаспаковки);
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаписатьФайлыКаталога(Поток, Знач Смещение, Знач Стр, Знач КаталогРаспаковки)

	Для каждого СтрФайл Из Стр.Строки Цикл
		Если СтрФайл.ЭтоКаталог Тогда
			ЗаписатьФайлыКаталога(Поток, Смещение, СтрФайл, ОбъединитьПути(КаталогРаспаковки, Стр.Имя));
		Иначе 
			ЗаписатьФайл(Поток, Смещение, СтрФайл, КаталогРаспаковки);
		КонецЕсли;
	КонецЦикла;

КонецПроцедуры // ЗаписатьФайлыКаталога()

Процедура ЗаписатьФайл(Поток, Знач Смещение, Знач Ветвь, Знач КаталогРаспаковки)

	БуферДанные = Новый БуферДвоичныхДанных(Ветвь.Размер);
	Поток.Перейти(Смещение + Ветвь.Смещение, ПозицияВПотоке.Начало);
	Поток.Прочитать(БуферДанные, 0, Ветвь.Размер);
	
	Ф = Новый Файл(КаталогРаспаковки);
	
	Если НЕ Ф.Существует() Тогда
		СоздатьКаталог(КаталогРаспаковки);
	КонецЕсли;
	
	ЗаписьДанных = Новый ЗаписьДанных(ОбъединитьПути(КаталогРаспаковки, Ветвь.Имя));
	ЗаписьДанных.ЗаписатьБуферДвоичныхДанных(БуферДанные);
	
	Сообщить("Записан файл " + ОбъединитьПути(КаталогРаспаковки, Ветвь.Имя));
...
Функция ПрочитатьИмяФайла(Поток, Смещение, Размер)

	БуферИмя = Новый БуферДвоичныхДанных(Размер);
	Поток.Перейти(Смещение, ПозицияВПотоке.Начало);
	Поток.Прочитать(БуферИмя, 0, Размер);

	Возврат ПеревестиБайтыВТекст(БуферИмя);

КонецФункции // ПрочитатьИмяФайла()

Функция ПеревестиБайтыВТекст(БинарныйБуфер)
...
КонецФункции // ПеревестиБайтыВТекст()
