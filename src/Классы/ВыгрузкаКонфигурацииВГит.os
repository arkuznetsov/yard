...
Процедура ОбработатьДанные() Экспорт
	...
	НастройкиПроектаЕДТСохранены = Ложь;
	Если ВремФайл.Существует() Тогда
		Если КонвертироватьВФорматЕДТ Тогда
			ВременныйКаталогСНастройками = "";
			НастройкиПроектаЕДТСохранены = СохранитьСлужебныеДанныеЕДТ(КаталогВыгрузки, ВременныйКаталогСНастройками);
		КонецЕсли;
	...
	Если КонвертироватьВФорматЕДТ Тогда
		...
		Если НастройкиПроектаЕДТСохранены Тогда
...
Процедура ОписаниеКоманды(Команда) Экспорт
	...
	Команда.Опция("rs remove-support", Ложь, "снять конфигурацию с поддержки")
	       .Флаг();
...
// Функция - определяет путь к 1cedtcli на основе явно заданного каталога EDT
// или результата поиска через edtfind
//
// Если задан ПутьКЕДТ - ищет 1cedtcli в указанном каталоге и выбрасывает исключение,
// если файл не найден. Иначе - вызывает edtfind и возвращает найденный путь
// (пустая строка, если 1cedtcli отсутствует в установке).
//
// Возвращаемое значение:
//  Строка - полный путь к 1cedtcli или пустая строка
//
Функция ОпределитьПутьКЕДТИКС()

	Если ЗначениеЗаполнено(ПутьКЕДТ) Тогда
		ИмяФайла = ?(ПараметрыПриложения.ЭтоWindows(), "1cedtcli.exe", "1cedtcli");
		ПолныйПуть = ОбъединитьПути(ПутьКЕДТ, ИмяФайла);

		ФайлCLI = Новый Файл(ПолныйПуть);
		Если НЕ ФайлCLI.Существует() Тогда
			ВызватьИсключение СтрШаблон(
				"Файл %1 не найден в каталоге ""%2"". Убедитесь, что EDT установлена корректно.",
				ИмяФайла,
				ПутьКЕДТ);
		КонецЕсли;
	...
	ПоискЕДТ = Новый ПоискEDT();
	РезультатПоиска = ПоискЕДТ.НайтиИнформациюОEDT(ВерсияЕДТ);
	Возврат РезультатПоиска.ПутьКcli;

КонецФункции // ОпределитьПутьКЕДТИКС()
...
Процедура СконвертироватьВФорматЕДТ(КаталогВФорматеКонфигуратора, КаталогВФорматеЕДТ, ВерсияЕДТ = Неопределено)
	...
	ПутьКЕДТИКС = ОпределитьПутьКЕДТИКС();

	Если ЗначениеЗаполнено(ПутьКЕДТИКС) Тогда
		КонвертироватьВФорматЕДТ_НоваяВерсияCLI(КаталогВФорматеКонфигуратора, КаталогВФорматеЕДТ, ПутьКЕДТИКС);
	Иначе
...
Процедура КонвертироватьВФорматЕДТ_СтараяВерсияCLI(КаталогВФорматеКонфигуратора, КаталогВФорматеЕДТ, ВерсияЕДТ)
	...
	МенеджерВР = Новый МенеджерВременныхФайлов();
	РабочийКаталогЕДТ = МенеджерВР.СоздатьКаталог("edt-ws");
	...
	ПараметрыЗапускаЕДТ.Добавить(РабочийКаталогЕДТ);
...
Процедура КонвертироватьВФорматЕДТ_НоваяВерсияCLI(КаталогВФорматеКонфигуратора, КаталогВФорматеЕДТ, ПутьКЕДТИКС)
	МенеджерВР = Новый МенеджерВременныхФайлов();
	РабочийКаталогЕДТ = МенеджерВР.СоздатьКаталог("edt-ws");

	Команда = Новый Команда();
	Команда.УстановитьКоманду(ПутьКЕДТИКС);
	Команда.УстановитьКодировкуВывода(КодировкаТекста.UTF8);
...
Процедура ВыгрузитьКонфигурациюВФайлыIBCMD(Знач КаталогВыгрузки)
	...
	Лог.Информация("[%1]: Используется утилита ibcmd", ТипЗнч(ЭтотОбъект));
...
Процедура ВыгрузитьКонфигурациюВФайлыКонфигуратор(Знач КаталогВыгрузки)
	...
	Конфигуратор.ВыгрузитьКонфигурациюВФайлы(КаталогВыгрузки, , , , , ПутьКСпискуОбъектов);
