...
Процедура ОбработатьДанные() Экспорт
	...
	Если ВремФайл.Существует() Тогда
		Если КонвертироватьВФорматЕДТ Тогда
			...
			ВременныйКаталогСНастройками = МенеджерВР.СоздатьКаталог("project-settings");
			ФС.КопироватьСодержимоеКаталога(КаталогНастроекПроекта, ОбъединитьПути(ВременныйКаталогСНастройками, ".settings"));
			ФайлПроекта = ОбъединитьПути(КаталогВыгрузки, ".project");
			КопироватьФайл(ФайлПроекта, ОбъединитьПути(ВременныйКаталогСНастройками, ".project"));
			НастройкиПроектаEDTСохранены = Истина;
		КонецЕсли;
	...
	Если КонвертироватьВФорматЕДТ Тогда
		...
		Если НастройкиПроектаEDTСохранены Тогда
			...
			ФС.КопироватьСодержимоеКаталога(ОбъединитьПути(ВременныйКаталогСНастройками, ".settings"), КаталогНастроекПроекта);
			ФайлПроекта = ОбъединитьПути(КаталогВыгрузки, ".project");
			УдалитьФайлы(ФайлПроекта);
			КопироватьФайл(ОбъединитьПути(ВременныйКаталогСНастройками, ".project"), ФайлПроекта);
		КонецЕсли;
	...
	ИначеЕсли НЕ НовыйРепозиторий Тогда
		Лог.Информация("[%1]: Создание ветки GIT ""%2"" в репозитарии ""%3""",
		               ...
		               ИмяВеткиГит,
		               ГитРепозиторий.ПолучитьРабочийКаталог());
		ГитРепозиторий.СоздатьВетку(ИмяВеткиГит);
	КонецЕсли;
