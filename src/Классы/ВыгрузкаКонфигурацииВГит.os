...
//
Функция ОпределитьПутьК1cedtcli()
...
Процедура СконвертироватьВФорматЕДТ(КаталогВФорматеКонфигуратора, КаталогВФорматеЕДТ, ВерсияЕДТ = Неопределено)
	...
	ПутьК1cedtcli = ОпределитьПутьК1cedtcli();

	Если ЗначениеЗаполнено(ПутьК1cedtcli) Тогда
		КонвертироватьВФорматЕДТ_НоваяВерсияCLI(КаталогВФорматеКонфигуратора, КаталогВФорматеЕДТ, ПутьК1cedtcli);
	Иначе
...
Процедура КонвертироватьВФорматЕДТ_НоваяВерсияCLI(КаталогВФорматеКонфигуратора, КаталогВФорматеЕДТ, ПутьК1cedtcli)
	МенеджерВР = Новый МенеджерВременныхФайлов();
	ВоркСпейсЕДТ = МенеджерВР.СоздатьКаталог("edt-ws");

	Команда = Новый Команда();
	Команда.УстановитьКоманду(ПутьК1cedtcli);
	Команда.УстановитьКодировкуВывода(КодировкаТекста.UTF8);
