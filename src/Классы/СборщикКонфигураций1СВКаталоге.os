...
Процедура ОбработатьДанные() Экспорт
	...
	Для Каждого ТекОписание Из ВерсииДляОбработки Цикл
		...
		НайденаКонфигурацияДляОбновления = Ложь;
		...
		Для Каждого ТекВерсия Из ТекОписание.ВерсииДляОбновления Цикл
			...
			Если ФайлКонфигурации.Существует() Тогда
				НайденаКонфигурацияДляОбновления = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если НайденаКонфигурацияДляОбновления Тогда
			...
			Сборщик.УстановитьПараметрОбработкиДанных("ПутьКОбновлению"      , ОбъединитьПути(ТекОписание.КаталогВерсии, "1cv8.cfu"));
