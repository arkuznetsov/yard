...
Функция ПолучитьВерсииПриложения(Знач АдресРесурса,
	...
	АдресРесурса = Служебный.ДобавитьПараметрЗапроса(АдресРесурса, "allUpdates=true");
...
Функция ВыполнитьЗапросНаСайт(Знач Сервер, Знач АдресРесурса, Знач ПутьКФайлуДляСохранения = "")
	...
	АдресЗапроса = Служебный.ДекодироватьРазделителиПараметровHTML(СтрШаблон("%1%2", Сервер, АдресРесурса));

	Ответ = Сессия.ВызватьМетод("GET", АдресЗапроса, ДопПараметры);
