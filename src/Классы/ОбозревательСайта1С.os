...
Функция ПолучитьВерсииПриложения(Знач АдресРесурса,
	...
	Если Совпадения.Количество() > 0 Тогда
		Для Каждого ТекСовпадение Из Совпадения Цикл
			...
			ВерсииДляОбновления = СтрРазделить(ТекСовпадение.Группы[6].Значение, ",", Ложь);
			
			МажорныеВерсии = Новый Соответствие();
			Для й = 0 По ВерсииДляОбновления.ВГраница() Цикл
				МажорнаяВерсия = СокрЛП(Лев(ВерсииДляОбновления[й], СтрНайти(ВерсииДляОбновления[й], ".") - 1));
				Если МажорныеВерсии[МажорнаяВерсия] = Неопределено Тогда
					МажорныеВерсии.Вставить(МажорнаяВерсия, Новый Массив());
				КонецЕсли;
				МажорныеВерсии[МажорнаяВерсия].Добавить(Сред(ВерсииДляОбновления[й], СтрНайти(ВерсииДляОбновления[й], ".") + 1));
			КонецЦикла;

			Для Каждого ТекЭлемент Из МажорныеВерсии Цикл
				ВремМассив = ТекЭлемент.Значение;
				Версии.СортироватьВерсии(ВремМассив, "УБЫВ");
				Для Каждого ТекЗначение Из ВремМассив Цикл
					ТекВерсия.ВерсииДляОбновления.Добавить(СтрШаблон("%1.%2", ТекЭлемент.Ключ, ТекЗначение));
				КонецЦикла;
			КонецЦикла;
...
Процедура СортироватьОписанияВерсийПоДате(ОписанияВерсий, Порядок = "Возр")
	...
	ТабДляСортировки.Сортировать(СтрШаблон("Дата %1", Порядок));
