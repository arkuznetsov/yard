...
Функция ПолучитьВерсииПриложения(Знач АдресРесурса,
	...
	Если Совпадения.Количество() > 0 Тогда
		Для Каждого ТекСовпадение Из Совпадения Цикл
			...
			Для й = 0 По ТекВерсия.ВерсииДляОбновления.ВГраница() Цикл
				ТекВерсия.ВерсииДляОбновления[й] = СокрЛП(ТекВерсия.ВерсииДляОбновления[й]);
			КонецЦикла;

			Версии.СортироватьВерсии(ТекВерсия.ВерсииДляОбновления, "УБЫВ");
